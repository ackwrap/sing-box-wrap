...
func (c *tls12TicketConn) Read(b []byte) (int, error) {
	for {
		...
		if !handshakeComplete {
			if c.underDecoded.Len() < 5 {
				continue
			}
			if !bytes.Equal(c.underDecoded.Bytes()[:3], []byte{0x16, 3, 3}) {
				c.underDecoded.Reset()
				return 0, errTLS12TicketAuthIncorrectMagicNumber
			}
			frameLength := 5 + int(binary.BigEndian.Uint16(c.underDecoded.Bytes()[3:5]))
			if frameLength > 64*1024 {
				c.underDecoded.Reset()
				return 0, errTLS12TicketAuthTooShortData
			}
			if c.underDecoded.Len() < frameLength {
				continue
			}
			frame := c.underDecoded.Next(frameLength)
			if len(frame) < 11+32+1+32 {
				return 0, errTLS12TicketAuthTooShortData
			}
			if !hmac.Equal(frame[33:43], c.hmacSHA1(frame[11:33])[:10]) || !hmac.Equal(frame[len(frame)-10:], c.hmacSHA1(frame[:len(frame)-10])[:10]) {
				return 0, errTLS12TicketAuthHMACError
			}
			c.stateMutex.Lock()
			_, err = c.writeLocked(nil)
			c.stateMutex.Unlock()
			if err != nil {
				return 0, err
			}
...
func (c *tls12TicketConn) writeLocked(b []byte) (int, error) {
	length := len(b)
